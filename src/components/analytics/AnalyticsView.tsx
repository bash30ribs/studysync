    ...
    <div className="p-4 lg:p-7 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="p-5 ui-card">
          ...
          <div className="flex items-baseline gap-1.5 mt-1.5">
            ...
            <span className="text-xs text-teal-700 dark:text-teal-300 font-semibold">Excellent</span>
          </div>
        ...
        <div className="p-5 ui-card">
          ...
          <div className="flex items-baseline gap-1.5 mt-1.5">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">overall cohort</span>
          </div>
          <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
        ...
        <div className="p-5 ui-card">
          ...
          <div className="flex items-baseline gap-1.5 mt-1.5">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">&lt; 25% submissions</span>
          </div>
      ...
      <div className="p-5 ui-card space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
          ...
          <span className="text-[11px] font-bold text-teal-700 dark:text-teal-300 bg-teal-500/15 px-2.5 py-1 rounded-full self-start">
            Peak: 9 PM — 11 PM IST
          </span>
        </div>
            ...
            <div key={i} className="flex flex-col items-center gap-1">
              <div 
                className={`w-full h-12 rounded-lg transition-all ${
                  h.intensity >= 8 ? 'bg-teal-500' :
                  h.intensity >= 5 ? 'bg-teal-500/70' :
                  h.intensity >= 3 ? 'bg-teal-500/40' :
                  h.intensity >= 1 ? 'bg-teal-500/20' :
            ...
            {[
              ...
              <div key={item.week} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                ...
                <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-t-lg h-32 flex items-end overflow-hidden">
                  <div
                    className="w-full bg-teal-500 rounded-t-lg progress-bar-fill hover:bg-teal-600 transition-all"
                    style={{ height: `${item.rate}%` }}
                  />
                </div>
                <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 text-center truncate">
                  {item.week}
                </span>
              </div>
            ))}
            ...
            {subjectStats.map(s => (
              <div key={s.subject} className="space-y-1.5">
                ...
                <div className="w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                  <div
                    className="h-full bg-teal-500 rounded-full progress-bar-fill"
                    style={{ width: `${s.rate}%` }}
                  />
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 ui-card">
          ...
          <div className="space-y-2">
            {mostMissed.map((asg, idx) => (
              <div
                key={asg.id}
                className="p-3 rounded-lg bg-slate-50/80 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 text-xs"
              >
                ...
                <span className="font-mono text-rose-600 dark:text-rose-400 font-bold shrink-0 bg-rose-50 dark:bg-rose-950/40 px-2 py-0.5 rounded">
                  {asg.missed} pending
                </span>
              </div>
            ))}
        ...
        <div className="p-5 ui-card">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="px-2 py-0.5 rounded bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 text-[10px] font-extrabold uppercase">
              Action Needed
            </span>
          </div>
          ...
          <div className="space-y-2">
            {ghostStudents.length === 0 ? (
              <p className="text-xs text-teal-600 dark:text-teal-400 py-6 text-center font-bold">
              ...
              ghostStudents.map(stu => (
                <div
                  key={stu.id}
                  className="p-3 rounded-lg bg-slate-50/80 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs"
                >
                  <div>
